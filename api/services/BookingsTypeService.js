module.exports = {
    find: function (ctx, filter, params) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var records = await BookingsType.find(qryObj).meta({makeLikeModifierCaseInsensitive: true});
            } catch (error) {
            ...
            if (params.totalCount) {
                try {
                    var totalRecords = await BookingsType.count(filter).meta({makeLikeModifierCaseInsensitive: true});
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
