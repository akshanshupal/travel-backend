module.exports = {
    find: function (ctx, filter, params) {
        return new Promise(async (resolve, reject) => {
            ...
            // if (filter.userAssigned && filter.userAssigned.trim()) filter.userAssigned = { contains: filter.userAssigned.trim() };
            ...
            try {
                var records = await Leads.find(qryObj);;
            } catch (error) {
            ...
            if (params.populate) {
                ...
                for (let ami = 0; ami < sails.models.leads.associations.length; ami++) {
                    assosiationModels[sails.models.leads.associations[ami].alias] = sails.models.leads.associations[ami].model;
                }
            ...
            if (params.totalCount) {
                try {
                    var totalRecords = await Leads.count(filter)
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
    ...
    findOne: function (ctx, id, params) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var record = await Leads.findOne(qryObj);;
            } catch (error) {
                return reject({ statusCode: 500, error: error });
            }
            if (!record) {
    ...
    create: function (ctx, data, avoidRecordFetch) {
        return new Promise(async (resolve, reject) => {
            ...
            if(data.campaign){
                const {data : campaign} = await CampaignService.findOne(ctx,data.campaign);
                data.pipeline = campaign.pipeline;
            }

            if (avoidRecordFetch) {
                try {
                    var record = await Leads.create(data);
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
            } else {
                try {
                    var record = await Leads.create(data).fetch();
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
            }
    ...
    updateOne: function (ctx, id, updtBody) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var record = await Leads.updateOne(filter).set(updtBody);
            } catch (error) {
                return reject({ statusCode: 500, error: error });
            }
