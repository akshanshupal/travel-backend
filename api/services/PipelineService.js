module.exports = {
    find: function (ctx, filter, params) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var records = await Pipeline.find(qryObj);;
            } catch (error) {
            ...
            if (params.populate) {
                ...
                for (let ami = 0; ami < sails.models.pipeline.associations.length; ami++) {
                    assosiationModels[sails.models.pipeline.associations[ami].alias] = sails.models.pipeline.associations[ami].model;
                }
            ...
            if (params.totalCount) {
                try {
                    var totalRecords = await Pipeline.count(filter)
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
    ...
    findOne: function (ctx, id, params) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var record = await Pipeline.findOne(qryObj);;
            } catch (error) {
                return reject({ statusCode: 500, error: error });
            }
            if (!record) {
    ...
    create: function (ctx, data, avoidRecordFetch) {
        return new Promise(async (resolve, reject) => {
            ...
            if (avoidRecordFetch) {
                try {
                    var record = await Pipeline.create(data);
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
            } else {
                try {
                    var record = await Pipeline.create(data).fetch();
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
            }
    ...
    updateOne: function (ctx, id, updtBody) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var record = await Pipeline.updateOne(filter).set(updtBody);
            } catch (error) {
                return reject({ statusCode: 500, error: error });
            }
    ...
    deleteOne: function (ctx, id) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                await Pipeline.destroyOne(filter);
            } catch (error) {
                return reject({ statusCode: 500, error: error });
            }
