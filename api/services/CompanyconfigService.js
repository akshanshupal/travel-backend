module.exports = {
    find: function (ctx, filter, params) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var records = await Companyconfig.find(qryObj);;
            } catch (error) {
            ...
            if (params.populate) {
                ...
                for (let ami = 0; ami < sails.models.companyconfig.associations.length; ami++) {
                    assosiationModels[sails.models.companyconfig.associations[ami].alias] = sails.models.companyconfig.associations[ami].model;
                }
            ...
            if (params.totalCount) {
                try {
                    var totalRecords = await Companyconfig.count(filter)
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
    ...
    findOne: function (ctx, id, params) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var record = await Companyconfig.findOne(qryObj);;
            } catch (error) {
                return reject({ statusCode: 500, error: error });
            }
            if (!record) {
    ...
    create: function (ctx, data, avoidRecordFetch) {
        return new Promise(async (resolve, reject) => {
            ...
            if (!data.panelUrl) {
                return reject({ statusCode: 400, error: { message: 'panelUrl is required!' } });
            }
            let existingPanelUrl;
            try {
                existingPanelUrl = await this.find(ctx, {panelUrl: data.panelUrl})
            } catch (error) {
                return reject({ statusCode: 400, error: { message: 'error in fetching existing company config' } });
            }
            if(existingPanelUrl?.length){
                return reject({ statusCode: 400, error: { message: 'panelUrl is already associalted with other company!' } });
            }

            if (avoidRecordFetch) {
                try {
                    var record = await Companyconfig.create(data);
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
            } else {
                try {
                    var record = await Companyconfig.create(data).fetch();
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
            }
    ...
    updateOne: function (ctx, id, updtBody) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var record = await Companyconfig.updateOne(filter).set(updtBody);
            } catch (error) {
                return reject({ statusCode: 500, error: error });
            }
    ...
    deleteOne: function (ctx, id) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                await Companyconfig.destroyOne(filter);
            } catch (error) {
                return reject({ statusCode: 500, error: error });
            }
