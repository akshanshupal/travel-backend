module.exports = {
    find: function (ctx, filter, params) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var records = await <%=modelProperName%>.find(qryObj);;
            } catch (error) {
            ...
            if (params.populate) {
                ...
                for (let ami = 0; ami < sails.models.<%=modelName%>.associations.length; ami++) {
                    assosiationModels[sails.models.<%=modelName%>.associations[ami].alias] = sails.models.<%=modelName%>.associations[ami].model;
                }
            ...
            if (params.totalCount) {
                try {
                    var totalRecords = await <%=modelProperName%>.count(filter)
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
    ...
    findOne: function (ctx, id, params) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var record = await <%=modelProperName%>.findOne(qryObj);;
            } catch (error) {
                return reject({ statusCode: 500, error: error });
            }
            if (!record) {
    ...
    create: function (ctx, data, avoidRecordFetch) {
        return new Promise(async (resolve, reject) => {
            ...
            if (avoidRecordFetch) {
                try {
                    var record = await <%=modelProperName%>.create(data);
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
            } else {
                try {
                    var record = await <%=modelProperName%>.create(data).fetch();
                } catch (error) {
                    return reject({ statusCode: 500, error: error });
                }
            }
    ...
    updateOne: function (ctx, id, updtBody) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                var record = await <%=modelProperName%>.updateOne(filter).set(updtBody);
            } catch (error) {
                return reject({ statusCode: 500, error: error });
            }
    ...
    deleteOne: function (ctx, id) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                await <%=modelProperName%>.destroyOne(filter);
            } catch (error) {
                return reject({ statusCode: 500, error: error });
            }
